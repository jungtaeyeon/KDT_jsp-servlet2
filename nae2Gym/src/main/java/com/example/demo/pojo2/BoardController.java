...
//			path = "board/boardList"; // -> /WEB-INF/jsp/ 이 경로에 있는 페이지로 이동하게 됨!
			} ////// end of 목록조회
		...
		// 등록일 때 - post 방식 - insert: 1(성공) or 0(실패)
			...
			int result = 0;
//			hmb.bind(pMap);
			// post방식은 매번 동일한 url을 요청하더라도 무조건 서버를 경유한다.
			// get방식은 같은 url 이면 인터셉트 당할 수 있다. 
			hmb.multiBind(pMap); // 첨부파일이 포함된 post방식일 때만 사용!! -> get방식은 무조건 첨부파일 안된다!!
			result = bLogic.boardInsert(pMap);
			if(result == 1) {
				path = "redirect:board/boardList";
			}else {
				path = "redirect:/board/boardError";
					}
			} ///////////////// end of Insert
		...
		// 수정일 때 - get,put 방식 - update: 1(성공) or 0(실패)
			...
			int result = 0;
			hmb.bind(pMap); // pMap.get(b_no) = 2가 나와야 함
			logger.info(pMap);
